...
class FDGUA():
    ...
    def __init__(self, model, adj, features,labels,idx_train,idx_val,idx_test,anchorsNum,dataset,device='cuda'):
        self.device = device
    ...
    def attack(self,max_epoch=5):
        ...
        file = open('results/coraResult/' + self.dataset + '_' + str(self.anchorsNum) + '.txt', 'w')
        target=0
        while target<int(self.labels.max())+1:
            ...
            while epoch<max_epoch:
                ...
                flipV1=self.vote(flipV,train_list,self.anchorsNum,target)
            ...
            file.write("target:{},best_epoch:{}\n".format(target,best_epoch))
            file.flush()
            target+=1
        file.close()
